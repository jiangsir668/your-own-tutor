...
---
name: "jiaocheng"
description: "教程大师 · 资料一站式生成客制化教程。Drop materials (PDF/PPT/Word), auto-generate personalized courses. v14 91.3分。触发词：「备课」「教我」「继续」「学习」「学到哪了」→ 读取progress.json报进度并直接续课。"
---
...
"备课"、"教我"、「学习」「学到哪了」「继续学」、用户上传了课件/教材/大纲等教学资料。

**「学习」「学到哪了」「继续学」= 1) Glob + 读取 progress.json；2) 一口报：第几章、第几个概念、第1章过了几个概念（具体名称）、有无暂挂；3) 直接进入阶段6 Step 1 开始教。不问"要继续吗"。不确认。直接开课。**

## 🔴 启动铁律

- **用户说「学习」「学到哪了」→ 立即Glob+读取 progress.json，一口报进度 + 直接进入阶段6 Step 1 开始教。**
...
- **「学习」「学到哪了」= Glob+读 progress，一口报进度（第几章、第几个概念、第1章过了哪些概念、有无暂挂）。然后直接续课。不问"要继续吗"。不确认。直接开课。**
...
| 19 | 进度报告只有数字没有概念名 | 列具体概念名，用户要知道学过什么 |
